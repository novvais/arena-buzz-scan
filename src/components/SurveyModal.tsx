...
export const SurveyModal = ({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) => {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-y-auto p-0 bg-card border-border">
        <div className="p-6 sm:p-8">
          ...
          {step === 1 && <PersonalInfoStep onSubmit={handlePersonalInfoSubmit} />}
          {step === 2 && <CategorySelection onSelect={handleCategorySelect} />}
          {step === 3 && selectedCategory && (
            <QuestionStep category={selectedCategory} onSubmit={handleQuestionSubmit} />
          )}
